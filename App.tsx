...
const App: React.FC = () => {
  ...
  const [showHolidays, setShowHolidays] = useLocalStorage<boolean>('vacationCalendarShowHolidays', true);
  ...
  const handleExportPdf = async () => {
    ...
    tempRoot.render(printElement);
    
    // Give React time to render and browser to paint before capturing
    await new Promise(resolve => setTimeout(resolve, 500));

    try {
        const { jsPDF } = jspdf;
        const canvas = await html2canvas(printContainer.children[0] as HTMLElement, { scale: 2 });
        ...
        pdf.save(`Prehlad_dovolenky_${name.replace(/\s+/g, '_')}_${currentYear}.pdf`);
    } catch (error) {
  ...
  return (
    <div id="app-container" className="min-h-screen bg-gray-100/80 text-gray-800 p-4 sm:p-6 lg:p-8 font-sans">
      <main className="max-w-7xl mx-auto bg-white/70 backdrop-blur-xl rounded-2xl shadow-2xl shadow-gray-300/40 p-6 sm:p-8 border border-gray-200/80">
        <Header
          name={name}
          setName={setName}
          organizationName={organizationName}
